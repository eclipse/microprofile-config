...
Microprofile Config does not contain an implementation itself but only provides the specified API and, a TCK and documentation.
...
A `Config` consists of the information collected from the registered `ConfigSources`.
These `ConfigSources` are sorted according to their _ordinal_.
That way it is possible to overwrite configuration from outside.
