...
Depending on the operating system type, environment variables may only use alphabetics or _ character while other characters are not always allowed. In order to set a value for a property with a name containing illegal characters as an environment variable, the following rules are used.
