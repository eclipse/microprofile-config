...
This `ConfigSource` searches 3 environment variables for a given property name (e.g. `com.ACME.size`):
  ...
  2. Replace the character that is neither alphabetic nor '_' with '_' (i.e. `com_ACME_size`)
  3. Replace the character that is neither alphabetic nor '_' with '_' and convert to upper case (i.e. `COM_ACME_SIZE`)
