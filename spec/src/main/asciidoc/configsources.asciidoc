...
Note that `config_ordinal` can be set within any `ConfigSource` implementation.  The default implementat of `getOrdinal()` will attempt to read this value, if found and a valid integer, the value will be used, otherwise the default of `100` will be returned.
